...
public class OTP {
    ...
    public static void main(String[] args) {
        try {
            ...
            // Инициализация TOTP
            TOTPGenerator otpGenerator = initTOTP(currentUser);
    ...
    private static User login() throws SQLException {
        ...
        User user = AUTHENTICATION_SERVICE.authenticate(username, password);
        if (user == null) {
    ...
    private static void registerUser(User creator) throws SQLException {
        ...
        boolean success = AUTHENTICATION_SERVICE.register(username, password, email, phone, telegramChatId, isAdmin, creator);
    ...
    private static TOTPGenerator initTOTP(User user) throws SQLException {
        if (user.isAdmin()) {
            System.out.println("1. Generate new secret key");
            System.out.println("2. Enter secret key");
            System.out.print("Choose your option: ");

            int choice = Integer.parseInt(OTP.scanner.nextLine());
            ...
            if (choice == 1) {
                ...
                String base32Key = OTP.scanner.nextLine();
    ...
    private static void showFileLogsMenu() {
        File logDir = new File(FileOTPService.OTP_LOG_DIR);
    ...
    private static void exportLogsToZip() {
        ...
        try {
            FileOTPService.exportLogsToZip(zipName);
